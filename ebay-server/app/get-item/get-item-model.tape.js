var test = require('tape')
var eBay = require('./get-item-model')

var data = `
<?xml version="1.0" encoding="utf-8"?>
<GetItemRequest xmlns="urn:ebay:apis:eBLBaseComponents">
  <RequesterCredentials>
    <eBayAuthToken>AgAAAA**AQAAAA**aAAAAA**NLqfWg**nY+sHZ2PrBmdj6wVnY+sEZ2PrA2dj6wFk4GoDJeDqA+dj6x9nY+seQ**G4IEAA**AAMAAA**</eBayAuthToken>
  </RequesterCredentials>
  <ItemID>110043671232</ItemID>
</GetItemRequest>
`

// valid request
test('eBay.getItem should return an error value of null', function (assert) {

  eBay.getItem(data, function(error, result){
    assert.error(error)
    assert.ok(result)
    assert.end()
  })

})


// invalid request
test('eBay.getItem should return an error', function (assert) {

  var _invalidData = "Very weird data here"

  eBay.getItem(_invalidData, function(error, result){
    assert.error(result)
    assert.ok(error)
    assert.end()
  })

})
